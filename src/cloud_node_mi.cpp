...
static rclcpp::Publisher s_output;
static rclcpp::Subscriber s_sub;
...
int main ( int argc , char ** argv )
{
  // Initialize the ROS system
    rclcpp::init ( argc , argv , "calculation_node") ;
...
//    priv_nh.param<bool>("use_laserSelection",use_laserSelection_,false);
  ...
  //ROS_INFO("%s : advertise name %s : %s",node_name.c_str(), advertise_name.c_str(), advertise_path.c_str() );
    ...
    RCLCPP_INFO(this->get_logger(),"model",device_model)
...
public:
    ...
    : Node("calculation_node")
  {
    ...
    this->get_parameter("use_gps", use_gps_);
    // Get other parameters here (e.g., data_level_, min_range, max_range, etc.)
    use_gps = use_gps_;

    // Advertise name
    std::string advertise_name = "rfans_points";
    this->declare_parameter("advertise_name", advertise_name);
    this->get_parameter("advertise_name", advertise_name);
    std::string advertise_path = "rfans_driver/" + advertise_name;

    // Subscribe name
    std::string subscribe_name = "rfans_packets";
    this->declare_parameter("subscribe_name", subscribe_name);
    this->get_parameter("subscribe_name", subscribe_name);
    std::string subscribe_path = "rfans_driver/" + subscribe_name;

    // Angle duration
    double angle_duration = 360.0;
    this->declare_parameter("angle_duration", angle_duration);
    this->get_parameter("angle_duration", angle_duration);
    SSBufferDec::SetAngleDuration(angle_duration);

    // Get other parameters and initialize gs_pointsPerRound and s_deviceType

    // Save xyz
    std::string save_xyz_key = node_name + "/save_xyz";
    std::string save_xyz_value = "no"; // Default: not save
    this->get_parameter_or(save_xyz_key, save_xyz_value, "no");
    SSBufferDec::setSaveXYZ(save_xyz_value == "yes");

    // Dynamic reconfigure server setup
    server_ = std::make_shared<dynamic_reconfigure::Server<rfans_driver::FilterParamsConfig>>(this);
    server_->setCallback(std::bind(&CalculationNode::callback, this, std::placeholders::_1, std::placeholders::_2));

    s_sub = this->create_subscription<rfans_driver::msg::RfansPacket>(
      subscribe_path, 30, std::bind(&CalculationNode::RFansPacketReceived, this, std::placeholders::_1));

    s_output = this->create_publisher<sensor_msgs::msg::PointCloud2>(advertise_path, RFANS_POINT_CLOUD_NUM);

    // Start any other threads or actions here
